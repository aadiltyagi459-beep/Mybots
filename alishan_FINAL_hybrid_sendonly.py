...
# =========================
# HYBRID MAIL SERVICE (Termux)
# =========================
# If set, the bot will call your Termux mail service instead of sending SMTP from Railway.
MAIL_SERVICE_URL = os.getenv("MAIL_SERVICE_URL", "").strip().rstrip("/")
...
VIDEO_FILE_ID_CREATE = "BAACAgUAAxkBAAMbaY8oLXSOkfkGZAmDv7u9992l8psAArUeAALKz3lU8YkQFjv1A2s6BA"
VIDEO_FILE_ID_LOGOUT = "BAACAgUAAxkBAAMdaY8opSnVs8DRUJMtnKXnbsZM7GkAArceAALKz3lU3I8XKyiyQVA6BA"
...
async def _termux_send_test_email(recipient: str, subject: str, body: str) -> bool:
    """Send an email via the external Termux mail service (HTTP).
    Returns True if the service reports success, else False.
    """
    if not MAIL_SERVICE_URL or not MAIL_SERVICE_SECRET:
        return False
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            r = await client.post(
                f"{MAIL_SERVICE_URL}/send",
                headers={"X-Secret": MAIL_SERVICE_SECRET},
...
async def smtp_bounce_check_fast(recipient: str, token: str) -> str:
    ...
    # Prefer external Termux mail service if configured (avoids SMTP blocks on Railway)
    if MAIL_SERVICE_URL and MAIL_SERVICE_SECRET:
        sent = await _termux_send_test_email(recipient, subject, body)
        return "ok_or_unknown" if sent else "unknown"
...
def get_profile_counts(user_id: int):
    con = db()
    cur = con.cursor()
    cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=?", (int(user_id),))
    total = int(cur.fetchone()["c"] or 0)
    cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=? AND state='approved'", (int(user_id),))
    approved = int(cur.fetchone()["c"] or 0)
    cur.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=? AND state='rejected'", (int(user_id),))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If user isn't in required channels yet, remember referrer and show join prompt
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == "👤 Profile":
        ...
        await update.message.reply_text(msg, reply_markup=ReplyKeyboardMarkup([["🔙 Back"]], resize_keyboard=True))
        return
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("REG_DONE:") or data.startswith("REG_CANCEL:") or data.startswith("REG_CANCEL_SURE:") or data.startswith("REG_CONFIRM:"):
        ...
        # timeout
        if not ok:
            # Only show TIME OUT after 20 hours (expires_at). Before that, allow user to retry later.
            if a and int(time.time()) > int(a["expires_at"]):
                try:
                    await q.edit_message_reply_markup(reply_markup=confirm_again_button(action_id))
                except Exception:
                    pass
                await q.message.reply_text("REGISTRATION CANCELED AUTOMATIC ⏰⌛")
        ...
        if data.startswith("REG_CANCEL_SURE:"):
            ...
            try:
                ...
                await q.message.reply_text("✅ Registration cancelled.")
        ...
        if data.startswith("REG_CONFIRM:"):
            # send to admin for approve/reject (admin panel)
            ...
            con.close()
                 ...
                 # Email check before admin request (fast SMTP/IMAP only)
            email = (r["email"] or "").strip()

            # Message we want to jump to when user taps the reply header
            target_msg_id = q.message.message_id

            # Send a linked 'checking' message (reply) so tap jumps back here
            ...
            # Heavy progress effect (same message edit)
            try:
                await wait_msg.edit_text("🔍 EMAIL CHECKING...\n▰▱▱▱▱▱▱▱▱▱ 10%")
            except Exception:
                pass
            ...
            token = f"{action_id}-{int(time.time())}"
            status = await smtp_bounce_check_fast(email, token)
            ...
            if status == "no_such_user":
                # ❌ NOT APPROVED: email does not exist
                # Keep CONFIRM AGAIN so user can retry within 20 hours
                ...
                try:
                    ...
                    pass
                try:
                    await wait_msg.edit_text("❌ NOT APPROVED: Email does not exist.")
                except Exception:
                    pass
                return

            # ✅ APPROVED: email check result is RIGHT
            try:
                await wait_msg.edit_text("✅ RIGHT (Email exists)")
            except Exception:
                pass
            ...
            # Add the extra note + keep only logout button on the same data message
            EXTRA_NOTE = (
                "\n\nFunds will be transferred to the main balance after 2-day hold.\n\n"
                "🚨 Be sure to LOG OUT of account on your device"
            )
            old_text = q.message.text or ""
            new_text = old_text + EXTRA_NOTE
            kb = InlineKeyboardMarkup([[
                InlineKeyboardButton("📲 How to logout of account ?", callback_data="VID_LOGOUT")
            ]])
            try:
            ...
            # Provisional HOLD credit (added immediately)
            con_pc = db()
            cur_pc = con_pc.cursor()
            cur_pc.execute(
                "SELECT action_id, hold_credit_id, amount, reverted FROM precredits WHERE action_id=?",
                (a["action_id"],)
            )
            pc = cur_pc.fetchone()

            if not pc:
                hid = add_hold_credit(user.id, PRE_CREDIT_AMOUNT)
                cur_pc.execute(
                    "INSERT INTO precredits(action_id, user_id, hold_credit_id, amount, created_at, reverted) "
                    "VALUES(?,?,?,?,?,0)",
                    (a["action_id"], user.id, hid, float(PRE_CREDIT_AMOUNT), int(time.time())),
                )
                con_pc.commit()

            con_pc.close()

            # Save into "form_table" (USERID | FIRSTNAME | EMAIL | PASSWORD | TIME)
            save_form_row(
                reg_id=a["reg_id"],
                user_id=user.id,
            ...
            # Auto-approve (no admin action needed)
            con2 = db()
            cur2 = con2.cursor()
            ...
            cur2.execute("SELECT COUNT(*) AS c FROM registrations WHERE user_id=? AND state='approved'", (a["user_id"],))
            approved_count = int(cur2.fetchone()["c"])
            paid_task = apply_task_rewards(cur2, a["user_id"], approved_count)

            # Referral bonus tracking (₹10 after 15 approved regs of referred user)
            cur2.execute("SELECT referrer_id FROM users WHERE user_id=?", (a["user_id"],))
            ur = cur2.fetchone()
            ...
            # Notify user about approval + task reward
            try:
                await context.bot.send_message(
                    chat_id=user.id,
                    text="✅ APPROVED (RIGHT)\nBALANCE UPDATED — CHECK YOUR BALANCE.",
                    reply_to_message_id=target_msg_id
                )
            except Exception:
                pass
            ...
            # Send logout tutorial automatically
            await send_logout_video(context, user.id)
            return
    # =========================
...
# Marker text for canceled registration
CANCEL_MARKER_TEXT = "\n________________________\nCANCELED ✖️ REGISTRATION"

# Helper to append cancel marker on the same message
async def append_cancel_marker(q):
    try:
        current_text = q.message.text or ""
        if "CANCELED ✖️ REGISTRATION" not in current_text:
            await q.edit_message_text(current_text + CANCEL_MARKER_TEXT)
    except Exception:
        pass

# Status labels for My Accounts
STATUS_LABELS = {
    "created": "CREATED",
    "approved": "APPROVED",
    "rejected": "REJECTED",
...
# Admin: Daily Total Approval (count of approved today)
def daily_total_approval(cur):
    today = datetime.date.today().isoformat()
    try:
        cur.execute(
            "SELECT COUNT(*) AS c FROM registrations "
            "WHERE state='approved' AND date(created_at,'unixepoch','localtime')=?",
            (today,)
        )
        row = cur.fetchone()
...
# --- Admin: Daily Total Approval (count of approved on a specific day offset) ---
def daily_total_approval_for_offset(cur, days_ago: int = 0) -> int:
    # counts approved registrations whose created_at date (localtime) matches today-days_ago
...
# ADMIN MENU WIRING
# =========================
ADMIN_DAILY_TOTAL_CB = "ADMIN_DAILY_TOTAL_APPROVAL"

def admin_menu_extra_kb():
    from telegram import InlineKeyboardMarkup, InlineKeyboardButton
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("☄️ Daily Total Approval", callback_data=ADMIN_DAILY_TOTAL_CB)
    ]])

async def handle_admin_daily_total(update, context):
    q = update.callback_query
    await q.answer()
    con = db()
    cur = con.cursor()
    total = daily_total_approval(cur)
    con.close()
    await q.edit_message_text(f"☄️ Daily Total Approval (Today): {total}")

# =========================
# HEAVY EFFECT HELPERS
# =========================
async def heavy_check_animation(bot, chat_id, reply_to_msg_id):
    steps = [
...
# =========================
# HEAVY EFFECT HELPERS (2,3,4)
# =========================

async def heavy_timeline_animation(bot, chat_id, reply_to_msg_id):
    steps = [
        "🟢 Created",
        "🟡 Email Checked",
        "🟠 Sent to Admin",
    ...
    msg = await bot.send_message(chat_id=chat_id, text="🧾 REGISTRATION TIMELINE", reply_to_message_id=reply_to_msg_id)
    for s in steps:
        try:
            await asyncio.sleep(0.6)
            await msg.edit_text(f"🧾 REGISTRATION TIMELINE\n\n{s}")
        except Exception:
            pass
    return msg

async def heavy_progress_bar(bot, chat_id, reply_to_msg_id, current, total):
    def bar(c,t):
        filled = int((c/t)*10) if t else 0
        return "█"*filled + "░"*(10-filled)
    msg = await bot.send_message(chat_id=chat_id, text=f"Progress: [{bar(current,total)}] {current}/{total}", reply_to_message_id=reply_to_msg_id)
    return msg

async def disable_buttons(q):
    try:
        await q.edit_message_reply_markup(reply_markup=None)
    except Exception:
        pass
...
# HOOK NOTES:
# - Call heavy_timeline_animation(...) after email-check success/reject.
# - Call heavy_progress_bar(...) in TASK menu rendering.
# - Call disable_buttons(q) right after CONFIRM AGAIN click to prevent spam.


# =========================
# HEAVY EFFECT HELPERS (5,6,7)
# =========================

def styled_card(title, lines, ok=True):
    bar = "━━━━━━━━━━━━━━━━"
    status = "✅" if ok else "❌"
    body = "\n".join(lines)
    return f"{bar}\n{status} {title}\n{bar}\n{body}\n{bar}"

async def notify_animated(bot, chat_id, texts, delay=0.6):
    # Sends one message and edits it step-by-step
    msg = await bot.send_message(chat_id=chat_id, text=texts[0])
    for t in texts[1:]:
        try:
...
async def admin_today_dashboard(bot, chat_id, stats: dict):
    lines = [
        f"📨 Email Checked: {stats.get('email_checked', 0)}",
        f"✅ Approved: {stats.get('approved', 0)}",
        f"❌ Rejected: {stats.get('rejected', 0)}",
        f"✖️ Canceled: {stats.get('canceled', 0)}",
    ]
    card = styled_card("TODAY STATS", lines, ok=True)
    return await bot.send_message(chat_id=chat_id, text=card)

# HOOK NOTES:
# - Use styled_card(...) to render success/failure results after checks.
# - Use notify_animated(...) for approve/reject/hold->main notifications.
# - Call admin_today_dashboard(...) from admin menu callback.
