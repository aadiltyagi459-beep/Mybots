...
VIDEO_CREATE_PATHS = ["/sdcard/CREATE/New.mp4", "New.mp4"]
VIDEO_LOGOUT_PATHS = ["/sdcard/LOGOUT/Old.mp4", "Old.mp4"]
# Cache Telegram file_id after first successful send (faster next time)
...
async def send_create_account_video(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ok = await _send_video_by_paths(context, chat_id, VIDEO_CREATE_PATHS, caption="✅ How to create account (video)", cache_key="create")
    if not ok:
        await context.bot.send_message(chat_id=chat_id, text="...")

async def send_logout_video(context: ContextTypes.DEFAULT_TYPE, chat_id: int):
    ok = await _send_video_by_paths(context, chat_id, VIDEO_LOGOUT_PATHS, caption="✅ How to logout of account (video)", cache_key="logout")
    if not ok:
        await context.bot.send_message(chat_id=chat_id, text="...")
...
PUBLIC_URL = os.environ.get("RAILWAY_STATIC_URL")  # Railway provides this
